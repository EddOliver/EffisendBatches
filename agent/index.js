...
const getBalanceTokens = tool(
  async ({ token }, { configurable: { address } }) => {
    ...
    const tokenSelected = tokens.findIndex(
      (tokenTemp) => tokenTemp.symbol.toLowerCase() === token.toLowerCase()
    );
    if (tokenSelected === -1) {
    ...
    if (tokenSelected === 0) {
      ...
      return JSON.stringify({
        ...
        message:"This is your current ETH balance on Base Mainnet.",
      });
    } else {
      ...
      return JSON.stringify({
        ...
        message:`This is your current ${tokens[tokenSelected].symbol} balance on Base Mainnet.`,
      });
    }
  },
  {
    name: "get_balance_base",
    description: `This tool retrieves the user's current X token balance, where X is any token on Base Mainnet. Use this when the user specifically asks for their token balance, 'token', "balance", or general wallet funds on Base Mainnet.
...
const transferTokens = tool(
  async ({ amount, to, token }, { configurable: { user } }) => {
    ...
    const { hash } = response;
    return JSON.stringify({
      message: "Transaction created and available on Base Mainnet.",
      status: "success",
      transaction: hash,
    });
  },
  {
    ...
    description: `This tool facilitates Tokens transfers on the Base Mainnet. It generates the transaction data for the user to sign. It activates whenever the user explicitly requests to send Tokens, initiates a transaction, or mentions terms like 'transfer,' 'Tokens,' or 'Base Mainnet' in relation to their wallet activity.
...
const swapTokens = tool(
  async ({ amount, fromToken, toToken }, { configurable: { user } }) => {
    ...
    return JSON.stringify({
      status: "success",
      message: "Your transaction is available on Base Mainnet.",
      transaction: hash,
    });
  },
  {
    name: "swap_tokens",
    description: `This tool facilitates token swaps on the Base Mainnet. It generates transaction data for the user to sign. It activates whenever the user explicitly requests to swap tokens, initiates a transaction, or mentions terms like 'swap,' 'token,' or 'Base Mainnet' in relation to their wallet activity.\n\nThe tool receives a token symbol like USDC, Base, etc...
...
// Transfer Base USDC to Kast Card - Modified version of transferTokens
const fundKastCard = tool(
  async ({ amount, address }, { configurable: { user } }) => {
    const response = await fetchURL(process.env.EXECUTE_PAYMENT_API, {
      user,
      amount,
      token: tokens.findIndex((tokenTemp) => tokenTemp.symbol === "USDC"),
      destination: address,
    });
    console.log(response);
    if (response === null) {
    ...
    return JSON.stringify({
      status: "success",
      message: "Your USDC has been transferred to your Kast Card.",
      transaction: hash,
    });
  },
  {
    name: "fund_kast_card",
    description:
      "This tool facilitates transfers where the specified amount is in USD, but the sending token is USDC on the Base Mainnet to USDC on the Kast Card. It generates transaction data for the user to sign and activates when the user explicitly opts to send USD to a Kast Card or mentions relevant terms such as 'transfer,' 'USDC,' 'Base Mainnet,' or 'Kast Card' in the context of wallet activity.",
    schema: z.object({
      amount: z.string(),
      address: z.string(),
    }),
...
// Transfer Base USDC to Linea USDC (Metamask Card) - Modified to return transaction data to API
const fundMetamaskCard = tool(
  async ({ amount, to }, { configurable: { user } }) => {
    ...
    return JSON.stringify({
      status: "success",
      message: "Your USDC has been transferred to your Metamask Card.",
      transaction: hash,
    });
  },
  {
    ...
    description:
      "This tool facilitates transfers where the specified amount is in USD, but the sending token is USDC on the Base Mainnet to USDC on Linea. It generates transaction data for the user to sign and activates when the user explicitly opts to send USD to a Metamask Card or mentions relevant terms such as 'transfer,' 'USDC,' 'Base Mainnet,' or 'Metamask Card' in the context of wallet activity.",
    schema: z.object({
      amount: z.string(),
      to: z.string(),
    }),
...
const listOfTools = tool(
  () => {
    ...
    return JSON.stringify({
      status: "info",
      message:
        "DeSmond can help you fund your Kast or Metamask card, transfer tokens on Base Mainnet, Swap tokens on Base Mainnet, and retrieve your current balance for any token on Base Mainnet.",
    });
  },
  {
    name: "list_of_tools",
